...
exports.description = 'Create a meme with one of the following patterns';
...
var matchers = [{
  ...
  pattern: /^(.+),? (so i got that going for me(?:,? which is nice)?)/i,
  ...
  run: function *() {
    if (this.$1) {
      ...
      if (image) {
        ...
        yield this.html('Sorry, your search didn\'t yield usable images for that meme');
      }
    }
  }
}];
...
function generate(url, text1, text2) {
  return function *() {
    ...
    if (json && json.imageUrl) {
      yield this.html('<img src="' + images.proxy(json.imageUrl) + '">');
    } else {
      yield this.fail();
    }
  };
