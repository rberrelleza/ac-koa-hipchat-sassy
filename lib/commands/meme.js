...
var debug = /\bmeme\b/i.test(process.env.NODE_DEBUG);
...
var matchers = [{
  ...
  run: function *() {
    if (this.$1) {
      ...
      if (image) {
        yield generate(image.link, this.$3, this.$4);
      } else {
...
function apiError(msg, json) {
  ...
  return 'Sorry, an error occurred using the memecaptain.com API';
}
  ...
  return function *() {
    ...
    if (url.match(/^https?:\/\//i)) {
      ...
      if (code === 202) {
        ...
        if (!pollUrl) {
          yield this.text(apiError('Poll URL not found'));
          return;
        }
        var resultUrl = yield pollForResult(pollUrl);
        if (!resultUrl) {
          logger.info('/meme timed out while polling ' + pollUrl);
          yield this.text('Sorry, but the /meme request timed out');
          return;
        }
...
function *pollForResult(pollUrl, attempt) {
  attempt = attempt || 1;
  if (attempt > 10) {
    return null;
  }
  var pollResponse = yield request.get({url: pollUrl, followRedirect: false});
  switch (pollResponse.statusCode) {
    case 200:
      var nextAttemptDelay = 250 * attempt;
      if (debug) {
        logger.info('/meme will poll', pollUrl, 'in', nextAttemptDelay, 'ms');
      }
      yield sleep(nextAttemptDelay);
      return yield pollForResult(pollUrl, attempt + 1);
    case 303:
      return pollResponse.headers.location;
    default:
      apiError('Unexpected response: ' + pollResponse.statusCode, pollResponse.body);
      return null;
    }
}
