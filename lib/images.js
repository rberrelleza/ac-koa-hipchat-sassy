  ...
  return function *() {
    ...
    if (image) {
      if (options.format === 'html') {
        ...
        var imageUrl = exports.ensureExt(image.unescapedUrl);
        var iconUrl = "http://www.google.com/favicon.ico";
        var title = image.contentNoFormatting || image.titleNoFormatting;
        if (this.sender.mention_name === "BarbarianLegendJohnStorm") {
          return yield this.image(imageUrl, iconUrl, title, {format: 'html'});
        } else {
